...
export default function ProjectDetail() {
  ...
  const project = useMemo(() => {
    if (!id) return null;
    return projects.find((p) => p._id?.toString() === id?.toString()) || null;
  }, [projects, id]);

  // Join socket room for real-time task updates
  useEffect(() => {
    if (id && joinProjectRoom) joinProjectRoom(id);
  }, [id, joinProjectRoom]);
  ...
  if (!project) {
    return (
      <div className="p-6 text-center">
        ...
        <button onClick={() => navigate('/projects')} className="px-4 py-2 rounded bg-zinc-200">
          Back to Projects
        </button>
      </div>
    );
    ...
    <div className="space-y-5 max-w-6xl mx-auto">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2 text-sm rounded bg-blue-600 text-white hover:bg-blue-700 transition"
        >
      ...
      <div className="flex gap-2 border rounded p-1 border-zinc-200 dark:border-zinc-800">
        {[
          ...
          <button
            ...
            className={`px-4 py-2 text-sm rounded transition ${activeTab === tabItem.key ? 'bg-zinc-200 dark:bg-zinc-700' : 'hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
          >
            {tabItem.label}
          </button>
        ))}
