...
export default function StatsGrid() {
    ...
    const statCards = [
        {
            ...
            subtitle: `projects in ${workspaceName}`, 
    ...
    useEffect(() => {
        // 2. Added checks to ensure projects is an array and populated
        if (currentWorkspace && Array.isArray(currentWorkspace.projects)) {
            setStats({
                totalProjects: currentWorkspace.projects.length,
                activeProjects: currentWorkspace.projects.filter(
                    (p) => p && p.status !== "CANCELLED" && p.status !== "COMPLETED"
                ).length,
                // Ensure tasks exist before running length or filter
                completedProjects: currentWorkspace.projects
                    .filter((p) => p && p.status === "COMPLETED")
                    .length, 
                myTasks: currentWorkspace.projects.reduce(
                    (acc, project) =>
                        acc + (project.tasks?.filter(
