...
app.post('/api/tasks', authenticate, async (req, res) => {
  try {
    ...
    const task = new Task({ title, description, type, status, priority, assignee: assigneeId || null, due_date, projectId });
    await task.save();
...
app.put('/api/tasks/:id', authenticate, async (req, res) => {
  try {
    const updated = await Task.findByIdAndUpdate(req.params.id, req.body, { new: true }).populate('assignee', '-password');
    const io = req.app.get('io');
...
app.delete('/api/tasks/:id', authenticate, async (req, res) => {
  try {
    ...
    if (task) {
      ...
      const io = req.app.get('io');
      io.to(`project:${task.projectId}`).emit('task:deleted', { taskId: req.params.id, projectId: task.projectId });
    }
...
app.post('/api/tasks/:taskId/comments', authenticate, async (req, res) => {
  try {
    ...
    const comment = new Comment({ taskId: req.params.taskId, userId: req.user.id, content, userName, userAvatar });
    await comment.save();
